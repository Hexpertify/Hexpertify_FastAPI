...
async def test_assign_role_to_user(client, admin_token):
    ...
    response = await client.post(f"/api/v1/users/{user_id}/roles", json={"role_id": role_id}, headers=headers)
...
async def test_assign_role_to_nonexistent_user_fails(client, admin_token):
    ...
    response = await client.post(f"/api/v1/users/{fake_user_id}/roles", json={"role_id": role_id}, headers=headers)
...
async def test_replace_user_roles(client, admin_token):
    ...
    await client.post(f"/api/v1/users/{user_id}/roles", json={"role_id": role_id_1}, headers=headers)
    response = await client.put(f"/api/v1/users/{user_id}/roles", json={"role_ids": [role_id_2]}, headers=headers)
    assert response.status_code == 200
    roles = response.json()
    assert len(roles) == 1
...
async def test_remove_user_role(client, admin_token):
    ...
    delete_resp = await client.delete(f"/api/v1/users/{user_id}/roles/{role_id}", headers=headers)
...
async def test_remove_unassigned_role_returns_404(client, admin_token):
    ...
    response = await client.delete(f"/api/v1/users/{user_id}/roles/{role_id}", headers=headers)
    assert response.status_code == 404