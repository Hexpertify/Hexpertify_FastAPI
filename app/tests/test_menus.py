...
async def test_create_root_menu(client):
...
async def test_create_child_menu(client):
...
async def test_create_duplicate_menu_code_fails(client):
    payload = {"code": "DUP_MENU", "name": "Dup Menu"}
    await client.post("/api/v1/menus/", json=payload)
    response = await client.post("/api/v1/menus/", json=payload)
    assert response.status_code == 409


async def test_create_menu_with_invalid_parent_fails(client):
...
async def test_list_menus(client):
    await client.post("/api/v1/menus/", json={"code": "LIST_MENU_TEST", "name": "List Menu Test"})
    response = await client.get("/api/v1/menus/")
    assert response.status_code == 200
    assert isinstance(response.json(), list)


async def test_menu_tree(client):
    parent_resp = await client.post("/api/v1/menus/", json={
...
async def test_update_menu(client):
    ...
    response = await client.put(f"/api/v1/menus/{menu_id}", json={"name": "After Update"})
    assert response.status_code == 200
    assert response.json()["name"] == "After Update"


async def test_menu_cannot_be_own_parent(client):
    create_resp = await client.post("/api/v1/menus/", json={
        "code": "SELF_PARENT_TEST", "name": "Self Parent Test"
    })
    menu_id = create_resp.json()["id"]

    response = await client.put(f"/api/v1/menus/{menu_id}", json={"parent_id": menu_id})
    assert response.status_code == 400
...
async def test_delete_menu(client):
    ...
    delete_resp = await client.delete(f"/api/v1/menus/{menu_id}")
