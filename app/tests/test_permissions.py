...
async def test_create_duplicate_permission_fails(client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    payload = {"code": "dup.perm", "name": "Dup Permission"}
    await client.post("/api/v1/permissions/", json=payload, headers=headers)
    response = await client.post("/api/v1/permissions/", json=payload, headers=headers)
    assert response.status_code == 409


async def test_get_permission_by_id(client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
...
async def test_list_permissions(client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    await client.post("/api/v1/permissions/", json={"code": "list.test", "name": "List Test"}, headers=headers)
...
async def test_update_permission(client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    create_resp = await client.post("/api/v1/permissions/", json={
    ...
    response = await client.put(f"/api/v1/permissions/{permission_id}", json={"name": "After Update"}, headers=headers)
    assert response.status_code == 200
    assert response.json()["name"] == "After Update"


async def test_delete_permission(client, admin_token):
    headers = {"Authorization": f"Bearer {admin_token}"}
    create_resp = await client.post("/api/v1/permissions/", json={
    ...
    delete_resp = await client.delete(f"/api/v1/permissions/{permission_id}", headers=headers)
