...
async def replace_user_roles(user_id: uuid.UUID, data: UserRoleBulkAssign, db: AsyncSession = Depends(get_db)):
    service = UserRoleService(db)
...
async def add_user_role(user_id: uuid.UUID, data: UserRoleAssign, db: AsyncSession = Depends(get_db)):
    service = UserRoleService(db)
    return await service.add_role(user_id, data.role_id)


@router.delete("/{user_id}/roles/{role_id}", status_code=204)
async def remove_user_role(user_id: uuid.UUID, role_id: uuid.UUID, db: AsyncSession = Depends(get_db)):
    service = UserRoleService(db)
